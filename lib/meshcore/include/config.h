...
#define PING_REPLY_DELAY_MS 900
...
// Размер блока файловой системы. Скачанный образ дополняется нулями до этой границы:
// последний неполный блок при записи из сетевой задачи не доезжает до флеша, и файл
// замирает на кратном размеру блока — образ уходил в эфир обрезанным.
