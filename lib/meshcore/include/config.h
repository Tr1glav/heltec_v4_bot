...
// Размер блока файловой системы. Скачанный образ копится в буфере и уходит на флеш только
// полными блоками: неполный блок в конце не доезжает до флеша, и файл замирает короче
// принятого. Остаток добивается нулями до полного блока — распаковщику узла лишние нули
// после сжатого потока безразличны, он останавливается на конце сжатых данных.
