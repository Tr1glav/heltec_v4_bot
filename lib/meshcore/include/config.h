...
// ===== IP OVER MESH (FEATURE_MESH_IP) =====
// Туннель сырых IP-датаграмм поверх сенсорного канала: компаньон раздаёт WiFi (AP),
// перехватывает исходящие IP-кадры телефона и гонит их координатору-шлюзу; координатор
// делает NAT и уходит в интернет своим STA WiFi. Ответы возвращаются тем же маршрутом.
//
// Формат кадра сенсорного канала (текстовый):
//   данные:  "ip:d" + 3 hex (seq, 12 бит) + 1 hex (fi<<4 | nf) + base64(фрагмент)
//   подтв.: "ip:a" + 3 hex (база окна) + 4 hex (bitmap 16 фрагментов)
//
// Надёжная доставка: одно сообщение = один пакет IP, фрагменты летят пачкой (≤ 15),
// приёмник держит окно MESH_IP_WINDOW фрагментов и подтверждает их bitmap16;
// ретрансмиссия неподтверждённых каждые MESH_IP_RTT_MS, сброс после MESH_IP_RETRY_MAX.
#define MESH_IP_PFX             "ip:"     // префикс сообщений туннеля в сенсорном канале
#define MESH_IP_QUEUE_MAX       8         // очередь исходящих IP-датаграмм (кольцо)
#define MESH_IP_PKT_MAX         1536      // размер буфера одной IP-датаграммы (MTU 1500)
