...
// секунды) идёт в счёт серии — одно «button», два «button2», три запускают проверку
// связи; от двух секунд переключается экран, а промежуток от секунды до двух намеренно
// не делает ничего, чтобы случайное удержание не уходило сообщением в сеть.
